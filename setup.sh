...
openssl req -x509 -newkey rsa:2048 -keyout docker/nginx/selfsigned-key.pem -out docker/nginx/selfsigned-cert.pem -days 3650 -nodes -subj "/CN=*.untuvaopintopolku.fi" -addext "subjectAltName=DNS:*.untuvaopintopolku.fi,DNS:*.hahtuvaopintopolku.fi,DNS:*.testiopintopolku.fi"
